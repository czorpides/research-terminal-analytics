...
import { NAV_GROUPS, ROUTES, STATUS_META, routesInGroup, type NavStatus, type RouteEntry } from "@/lib/navigation/routes";
      ...
      {activeOnly.map((item) => {
        ...
        return (
          <Link key={item.path} to={item.path}
            className={cn("shrink-0 px-3 py-2 font-mono text-[10px] uppercase tracking-wider",
      ...
      {ROUTES.map((r) => {
        ...
        const inner = (
          <div className={cn(
            ...
            active ? "bg-sidebar-accent text-[var(--primary)]" : isReal ? "text-muted-foreground hover:bg-sidebar-accent/50 hover:text-foreground" : "text-muted-foreground/40",
          )}>
        ...
        return (
          <Tooltip key={r.id}>
            ...
            <TooltipContent side="right" className="text-xs">
              ...
              <div className="text-[10px] text-muted-foreground">{status.label} · Stage {r.stage}</div>
            </TooltipContent>
          </Tooltip>
        );
      })}
...
function NavGroupSection({ groupKey, pathname }: { groupKey: RouteEntry["group"]; pathname: string }) {
  const group = NAV_GROUPS.find((g) => g.key === groupKey)!;
  const items = routesInGroup(groupKey);
  const containsActive = items.some((r) => (r.path === "/" ? pathname === "/" : pathname.startsWith(r.path)));
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen} className="mb-1">
      <CollapsibleTrigger className="group flex w-full items-center gap-1.5 rounded-sm px-2 py-1.5 text-muted-foreground hover:bg-sidebar-accent/40 hover:text-foreground">
        ...
        <span className="flex-1 text-left font-mono text-[10px] uppercase tracking-[0.15em]">{group.label}</span>
        <span className="font-mono text-[9px] text-muted-foreground/60">{group.code}</span>
      </CollapsibleTrigger>
      <CollapsibleContent>
        <div className="mt-0.5 space-y-0.5 pl-3">
          {items.map((r) => <NavItem key={r.id} r={r} pathname={pathname} />)}
        </div>
      </CollapsibleContent>
    </Collapsible>
  );
...
function statusDot(status: NavStatus) {
  switch (status) {
    case "active":         return "bg-[var(--positive)]";
    case "shadow":         return "bg-[var(--primary)]";
    case "in_development": return "bg-yellow-500";
    case "awaiting_data":  return "bg-muted-foreground/50";
    case "planned":        return "bg-muted-foreground/30";
  }
}
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent side="right" className="max-w-[260px] text-xs">
        <div className="font-medium">{r.name}</div>
        <div className="mt-0.5 text-[10px] text-muted-foreground">{status.label} · Stage {r.stage}</div>
