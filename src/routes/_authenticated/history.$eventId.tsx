...
const detailQueryOptions = (code: string) =>
  queryOptions({
    ...
    meta: [
      {
        title: loaderData?.event
          ? `${loaderData.event.name} — Historical Event`
          : "Historical Event",
      },
  ...
  notFoundComponent: () => (
    <AppShell>
      <SectionHeader
        ...
        purpose="No historical event with that code."
      />
    </AppShell>
  ),
  errorComponent: ({ error }) => (
    <AppShell>
      <SectionHeader code="HE · error" title="Error loading event" purpose={error.message} />
    </AppShell>
  ),
});
...
function EventPage() {
  ...
  const e = data.event! as unknown as {
    ...
    narrative_issues: string[] | null;
    narrative_attempts: number | null;
  };
  const fp = (e.fingerprint ?? {}) as Record<string, string>;
  const citations = e.citations ?? [];
  const badgeClass =
    e.narrative_status === "verified"
      ? "text-emerald-400 border-emerald-500/40"
      : e.narrative_status === "needs_review"
  ...
  return (
    <AppShell>
      ...
      <section className="mt-4 rounded border border-border/50 bg-card/40 p-4 text-xs text-muted-foreground leading-relaxed">
        ...
        Every historical event is described in plain English across five sections — <em>Summary</em>
        , <em>Causes</em>, <em>Mechanism</em>, <em>What happened next</em>, <em>Key takeaway</em> —
        and every claim traces back to the <em>Citations</em> below. The <em>Verification</em> block
        shows how the narrative was checked: algorithm (structure + trusted-publisher allowlist) →
        API (link liveness) → AI (coherence). If AI can&rsquo;t verify, the loop rewrites the fields
        grounded in the citations and re-checks up to two more times before marking{" "}
        <em>needs review</em>.
      </section>

      {/* Verification status */}
      <section
        className={`mt-3 rounded border ${badgeClass} bg-card/30 p-3 flex items-center justify-between text-xs`}
      >
        <div>
          ...
          {e.narrative_confidence !== null && (
            <span className="ml-2 font-mono">· AI confidence {e.narrative_confidence}/100</span>
          )}
          {e.narrative_verified_at && (
            <span className="ml-2 text-muted-foreground">
              · last checked {new Date(e.narrative_verified_at).toLocaleString()}
            </span>
          )}
          {e.narrative_attempts ? (
            <span className="ml-2 text-muted-foreground">
              · {e.narrative_attempts} rewrite pass{e.narrative_attempts === 1 ? "" : "es"}
            </span>
          ) : null}
        </div>
      </section>
      ...
      <div className="mt-3">
        <ResearchNarrative
          summary={
            e.key_takeaway ??
            e.summary ??
            `${e.name} is recorded as a ${e.category.replaceAll("_", " ")} event.`
          }
          detail={
            e.what_happened_next ?? "The event narrative is awaiting a verified outcome summary."
          }
          watch={(e.tags ?? [])
            .slice(0, 4)
            .map((tag) => `Research theme: ${tag.replaceAll("_", " ")}`)}
          asOf={e.narrative_verified_at ?? e.end_date ?? e.start_date}
          confidence={e.narrative_confidence}
        />
      </div>
      ...
      <div className="grid gap-6 mt-6">
        ...
        <NarrativeBlock
          title="Causes"
          body={e.causes}
          placeholder="Awaiting narrative verification pass."
        />
        <NarrativeBlock
          title="Mechanism (how it transmitted through the system)"
          body={e.mechanism}
          placeholder="Awaiting narrative verification pass."
        />
        <NarrativeBlock
          title="What happened next"
          body={e.what_happened_next}
          placeholder="Awaiting narrative verification pass."
        />
        <NarrativeBlock
          title="Key takeaway"
        ...
        <section className="rounded border border-border/50 bg-card/50 p-4">
          ...
          {e.narrative_issues && e.narrative_issues.length > 0 && (
            <div className="mt-3 text-[11px] text-amber-400/80">
              Open issues: {e.narrative_issues.join(", ")}
            </div>
          )}
        </section>
        ...
        <section className="rounded border border-border/50 bg-card/50 p-4">
          <div className="text-xs text-muted-foreground uppercase tracking-wider mb-2">
            Economic environment profile — what each condition means
          </div>
              ...
              <tbody>
                {data.impacts.map((im, i) => (
                  <tr key={i} className="border-b border-border/20">
                    ...
                    <td
                      className={`text-right font-mono ${im.return_pct >= 0 ? "text-emerald-400" : "text-rose-400"}`}
                    >
                      {im.return_pct >= 0 ? "+" : ""}
                      {im.return_pct}%
                    </td>
