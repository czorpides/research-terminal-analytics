...
export interface AnalogMatch {
  ...
  /** Similarity reduced when the live environment profile is incomplete. */
  coverageAdjustedPct: number;
  reliabilityLabel: "strong" | "useful" | "weak";
  dimensionScores: Array<{
    dimension: keyof Fingerprint;
    current: string;
    historical: string;
    score: number;
  }>;
...
export async function findAnalogs(opts: MatchOptions = {}): Promise<MatchResult> {
  ...
  const dimKeys: (keyof Fingerprint)[] = [
    "rate_level",
    "rate_direction",
    "curve",
    "inflation",
  ...
  const scored: AnalogMatch[] = ((events ?? []) as HistoricalEventRow[]).map((e) => {
    let weightSum = 0,
      ...
      matched = 0,
      compared = 0;
    const dimensionScores: AnalogMatch["dimensionScores"] = [];
    ...
    for (const k of dimKeys) {
      ...
      dimensionScores.push({ dimension: k, current: cur, historical: ev, score: s });
      if (s === 1) matched += 1;
      weightSum += WEIGHTS[k];
      gotSum += s * WEIGHTS[k];
    }
    ...
    const coverageAdjustedPct = pct * current.coverage;
    const reliabilityLabel =
      coverageAdjustedPct >= 65 ? "strong" : coverageAdjustedPct >= 45 ? "useful" : "weak";
    let impactsForEvent = impactsByEvent.get(e.id) ?? [];
    if (opts.scopeFilter) {
      impactsForEvent = impactsForEvent.filter(
        (im) =>
          im.scope_type === opts.scopeFilter!.scope_type &&
          im.scope_code === opts.scopeFilter!.scope_code,
      );
    }
    return {
      event: e,
      dimsMatched: matched,
      dimsCompared: compared,
      matchPct: pct,
      coverageAdjustedPct,
      reliabilityLabel,
      dimensionScores,
      tagsMatched,
      impacts: impactsForEvent,
    };
  });
  ...
  filtered.sort((a, b) => b.coverageAdjustedPct - a.coverageAdjustedPct || b.matchPct - a.matchPct);
  const limit = opts.limit ?? 5;

  return {
    fingerprint: current,
    analogs: filtered.slice(0, limit),
    computedAt: new Date().toISOString(),
  };
}

/** Formatted one-line historical parallel bullet for a sector. Returns null if no strong analog. */
export async function historicalParallelBullet(
  scopeType: "sector" | "commodity",
  scopeCode: string,
  kind: "under" | "over",
): Promise<string | null> {
  const { analogs } = await findAnalogs({
    scopeFilter: { scope_type: scopeType, scope_code: scopeCode },
    minMatchPct: 50,
    limit: 3,
  });
  const dependable = analogs.filter((analog) => analog.coverageAdjustedPct >= 40);
  if (dependable.length === 0) return null;
  const relevant = dependable.map((a) => {
    const imp = a.impacts[0];
    return {
      name: a.event.name,
      pct: imp.return_pct,
      window: imp.window_days,
      adjustedPct: a.coverageAdjustedPct,
    };
  });
  const bestFit = relevant[0];
  const sameDir =
    kind === "under" ? relevant.filter((r) => r.pct > 0) : relevant.filter((r) => r.pct < 0);
  const pool = sameDir.length > 0 ? sameDir : relevant;
  const median = pool.map((r) => r.pct).sort((a, b) => a - b)[Math.floor(pool.length / 2)];
  return `Historical comparison — ${bestFit.name} was ${bestFit.adjustedPct.toFixed(0)}% similar after allowing for current-data coverage; the median ${bestFit.window}-day forward sector return was ${median > 0 ? "+" : ""}${median.toFixed(0)}% across ${pool.length} comparable episode${pool.length === 1 ? "" : "s"}.`;
}
