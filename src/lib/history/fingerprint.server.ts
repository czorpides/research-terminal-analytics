...
export const FINGERPRINT_VERSION = "history.environment_profile.v0.2";
...
export async function computeCurrentFingerprint(): Promise<FingerprintDetail> {
  ...
  // Load the newest history per series. A single oldest-first global limit can
  // exclude recent observations when several long histories share the query.
  const pointGroups = ids.length
    ? await Promise.all(
        ids.map(async (id) => {
  ...
  // Commodity: WTI
  const { data: wti } = await supabaseAdmin
    .from("commodities")
    .select("id, code")
    .eq("code", "WTI")
    .maybeSingle();
  ...
  if (wti) {
    const { data: cp } = await supabaseAdmin
      ...
      .select("ts, price")
      .eq("commodity_id", wti.id)
      .order("ts", { ascending: false })
      .limit(400);
    wtiSeries = (cp ?? []).reverse().map((r) => ({ asOf: r.ts as string, value: Number(r.price) }));
  }

  const last = (s?: Array<{ asOf: string; value: number }>) =>
  ...
  if (ten) {
    inputs.rate_10y = ten.value;
    asOf.rate_10y = ten.asOf;
