...
async function sourceIdFor(providerCode: string): Promise<string | null> {
  const { data } = await supabaseAdmin.from("data_sources").select("id").eq("provider_code", providerCode).maybeSingle();
...
export async function runEquityIngest(symbol: string): Promise<EquityIngestResult> {
  ...
  const { data: run } = await supabaseAdmin.from("ingestion_runs").insert({
    source_id: bookkeepingSourceId, data_category: "price_daily", status: "running", details: { symbol },
  }).select("id").single();
  ...
  try {
    ...
    if (quality.blocked) {
      ...
      return { status: "failed", rowsInserted: 0, runId, symbol, provider, quality, attempts, error: "quality_gate_blocked" };
    }
    ...
    if (fresh.length > 0) {
      const rows = fresh.map((b) => ({
        ...
        adj_close: b.adjClose ?? b.close, volume: b.volume,
    ...
    if (latest) {
      ...
      crossVerify = { verifier: cv.verifier, agrees: cv.agrees, detail: cv.detail, delta: cv.delta };
      const ageSec = Math.max(0, Math.floor((Date.now() - new Date(`${latest.date}T21:00:00Z`).getTime()) / 1000));
      computeConfidence({
        tier: "tier2_regulated", category: "price_daily", ageSeconds: ageSec,
    ...
    return { status: "success", rowsInserted: inserted, runId, symbol, provider, quality, attempts, crossVerify };
  } catch (e) {
    await supabaseAdmin.from("ingestion_runs").update({
...
export async function runAllEquityIngest(): Promise<EquityIngestResult[]> {
  ...
  for (const s of STOOQ_UNIVERSE) {
    try { out.push(await runEquityIngest(s.symbol)); }
