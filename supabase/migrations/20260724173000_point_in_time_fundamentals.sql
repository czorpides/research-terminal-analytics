-- Point-in-time statement storage for auditable Piotroski and Magic Formula
-- calculations. Existing fundamentals_annual / fundamentals_quarterly tables
-- keep only one value per period and cannot preserve filing revisions.
...
-- One row per live asset/metric avoids repeatedly transferring the append-only
-- data_points history whenever cross-sectional ranks are recomputed.
